...
void VM_Init( void ) {
	...
	Cvar_Get( "vm_ui", "0", CVAR_ARCHIVE );		// native DLL -- need modded UI code
...
int QDECL VM_DllSyscall( int arg, ... ) {
  // On x86_64, arguments are passed in registers (AMD64 ABI), so the
  // original &arg trick of walking the stack doesn't work.
  // Use va_args on all platforms to correctly capture arguments.
  intptr_t args[16];
  int i;
  va_list ap;
